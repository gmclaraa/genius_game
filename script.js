...
const padListener = (e) => {
    ...
    const pad = e.currentTarget; // captura o elemento enquanto ainda é válido

    let soundId;
    _gui.pads.forEach((p, key) => {
        if (p === pad)
            soundId = key;
    });
    ...
    setTimeout(() => {
        pad.classList.remove('game__pad--active') // usa a variável, não e.currentTarget
